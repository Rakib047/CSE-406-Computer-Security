...
def AES_Encryption(plain_text, round_keys,mainIV):
    ...
    for i in range(int(factor)):
        ...
        xor_block = bytearray([a ^ b for a, b in zip(plain_text[i*BLOCK_SIZE:(i+1)*BLOCK_SIZE], xor_block)])
        encrypted_block = AES_Encrypt(xor_block, round_keys)
        cipher_text += encrypted_block
        xor_block = encrypted_block  # Update xor_block for the next iteration

    if len(plain_text) % BLOCK_SIZE != 0:
        # If there is a partial block, pad and XOR with the previous ciphertext block or IV
        padded_text = padd_text(plain_text[factor*BLOCK_SIZE:])
        xor_block = bytearray([a ^ b for a, b in zip(padded_text, xor_block)])
        cipher_text += AES_Encrypt(xor_block, round_keys)
...
def AES_Decryption(cipher_text, round_keys,mainIV):
    ...
    for i in range(int(factor)):
        ...
        plain_text += bytearray([a ^ b for a, b in zip(decrypted_block, xor_block)])
        xor_block = cipher_text[i*BLOCK_SIZE:(i+1)*BLOCK_SIZE]  # Update xor_block for the next iteration
...
if __name__ == "__main__":
    ...
    if len_k not in [16, 24, 32]:
        ...
        start = time.time()
        cipher,iv = AES_Encryption(plain_text, round_keys)
