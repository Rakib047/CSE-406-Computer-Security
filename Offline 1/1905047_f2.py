...
def AES_Encryption(plain_text, round_keys):
    ...
    for i in range(int(factor)):
        counter_block = iv + bytes([(i >> shift) & 0xFF for shift in range(8 * (BLOCK_SIZE - len(iv) - 1), -1, -8)])
        encrypted_counter_block = AES_Encrypt(counter_block, round_keys)
        # XOR the encrypted counter block with the plaintext block
        cipher_text += bytearray([a ^ b for a, b in zip(plain_text[i * BLOCK_SIZE:(i + 1) * BLOCK_SIZE], encrypted_counter_block)])

    if len(plain_text) % BLOCK_SIZE != 0:
        # If there is a partial block, encrypt the counter block and XOR with the plaintext block
        counter_block = iv + bytes([(factor >> shift) & 0xFF for shift in range(8 * (BLOCK_SIZE - len(iv) - 1), -1, -8)])
        encrypted_counter_block = AES_Encrypt(counter_block, round_keys)
        padded_text = padd_text(plain_text[factor * BLOCK_SIZE:])
        cipher_text += bytearray([a ^ b for a, b in zip(padded_text, encrypted_counter_block)])
...
def AES_Decryption(cipher_text, round_keys):
    ...
    for i in range(int(factor)):
        counter_block = iv + bytes([(i >> shift) & 0xFF for shift in range(8 * (BLOCK_SIZE - len(iv) - 1), -1, -8)])
        encrypted_counter_block = AES_Encrypt(counter_block, round_keys)
        decrypted_block = bytearray([a ^ b for a, b in zip(cipher_text[i * BLOCK_SIZE:(i + 1) * BLOCK_SIZE], encrypted_counter_block)])
        plain_text += decrypted_block

    if len(cipher_text) % BLOCK_SIZE != 0:
        # If there is a partial block, encrypt the counter block and XOR with the ciphertext block
        counter_block = iv + bytes([(factor >> shift) & 0xFF for shift in range(8 * (BLOCK_SIZE - len(iv) - 1), -1, -8)])
        encrypted_counter_block = AES_Encrypt(counter_block, round_keys)
        plain_text += bytearray([a ^ b for a, b in zip(cipher_text[factor * BLOCK_SIZE:], encrypted_counter_block)])
